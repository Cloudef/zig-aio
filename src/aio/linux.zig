...
pub fn immediate(comptime len: u16, work: anytype) aio.ImmediateError!aio.CompletionResult {
    ...
    defer io.deinit();
    inline for (&work.ops) |*op| try uring_queue(&io, op, @intFromPtr(op));
    ...
    while (num > 0) {
        ...
        for (cqes[0..n]) |*cqe| {
            const op = blk: {
                inline for (&work.ops) |*op| if (@intFromPtr(op) == cqe.user_data) break :blk op;
                unreachable;
            };
...
inline fn uring_queue(io: *std.os.linux.IoUring, op: anytype, user_data: u64) aio.QueueError!void {
    var sqe = switch (comptime Operation.tagFromPayloadType(@TypeOf(op.*))) {
        ...
        .close => try io.close(user_data, op.file.handle),
        .timeout => try io.timeout(user_data, @ptrCast(&op.ts), 0, 0),
        .timeout_remove => try io.timeout_remove(user_data, @intFromEnum(op.id), 0),
        .link_timeout => try io.link_timeout(user_data, @ptrCast(&op.ts), 0),
        .cancel => try io.cancel(user_data, @intFromEnum(op.id), 0),
        .rename_at => try io.renameat(user_data, op.old_dir.handle, op.old_path, op.new_dir.handle, op.new_path, 0),
...
inline fn uring_handle_completion(op: anytype, cqe: *std.os.linux.io_uring_cqe) !void {
    ...
    if (err != .SUCCESS) {
        var skip_err = false;
        if (op.out_error) |out_error| {
            out_error.* = switch (comptime Operation.tagFromPayloadType(@TypeOf(op.*))) {
                .fsync => switch (err) {
                ...
                .close, .close_socket => unreachable,
                .timeout => switch (err) {
                    .SUCCESS, .INTR, .INVAL, .AGAIN => unreachable,
                    .TIME => blk: {
                        skip_err = true;
                ...
                .timeout_remove => switch (err) {
                    .SUCCESS, .INTR, .INVAL, .AGAIN => unreachable,
                    .BUSY => error.InProgress,
        ...
        if (!skip_err) return error.OperationFailed;
    }
